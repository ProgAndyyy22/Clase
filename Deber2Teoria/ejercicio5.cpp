...
<<<<<<< HEAD
int main() {
    const double OZ_A_ML = 29.5735;

    // --- Capacidad de cada bebida servida, en ml ---
    const double MASTER_VODKA_ML   = 11.0   * OZ_A_ML; // 7 partes
    const double BRISA_CARIBE_ML   = 9.0    * OZ_A_ML; // 9 partes
    const double GOLPE_BLANCO_ML   = 6.5    * OZ_A_ML; // 9 partes
    const double JUGO_NARANJA_ML   = 16.0   * OZ_A_ML; // jugo puro
    const double COLA_ML           = 13.0   * OZ_A_ML; // cola pura

    // --- Precio de venta de cada bebida ---
    const double PRECIO_MASTER_VODKA = 7.00;
    const double PRECIO_BRISA_CARIBE = 6.50;
    const double PRECIO_GOLPE_BLANCO = 7.50;
    const double PRECIO_JUGO_NARANJA = 3.00;
    const double PRECIO_COLA         = 1.00;

    // --- Botellas: precio y capacidad ---
    const double PRECIO_BOT_VODKA = 23.00, CAP_BOT_VODKA = 700.0;
    const double PRECIO_BOT_RON   = 12.00, CAP_BOT_RON   = 750.0;
    const double PRECIO_BOT_JUGO  = 6.00,  CAP_BOT_JUGO  = 2000.0;
    const double PRECIO_BOT_COLA  = 1.50,  CAP_BOT_COLA  = 1750.0;
    ...
    // e) Ganancia o perdida
    double ganancia = precioTotal - costoPedido;

    cout << "\n--- Resultados ---\n";
    cout << "a) Precio total de la orden: $" << precioTotal << endl;

    cout << "b) Botellas a comprar -> "
         << "Vodka: " << botVodka
         << ", Ron: " << botRon
         << ", Jugo: " << botJugo
         << ", Cola: " << botCola << endl;

    cout << "c) Costo de preparar el pedido: $" << costoPedido << endl;
    ...
    cout << "e) Ganancia (si es negativo, es perdida): $" << ganancia << endl;

    return 0;
}

// Aprendizaje adquirido: practique como expresar un problema con reglas de proporcion
// (partes de cada coctel) unicamente con expresiones aritmeticas, sin condicionales ni
// bucles, y como usar funciones matematicas como ceil() para redondear hacia arriba sin
// necesitar la instruccion if.
...
>>>>>>> 9fa5aaeaee283c7ec7df72ae609fd17a79d2fa48